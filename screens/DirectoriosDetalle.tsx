...
export default DirectoriosDetalle = ({ navigation, route }) => {
    ...
    useEffect(() => {
        fetch(global.UBYMED_WS_BASE + 'api/usuario', {
            ...
            body: JSON.stringify({
                action: "getDirectoriosCategoriaDetalle",
                id: idDetalle,
            })
        ...
        <View style={styles.containerServicios}>
...

            {
                dataMostrar.map((item) => {
                    return (
                        <React.Fragment key={item["directorio"]}>

                            <View style={styles.servicioMainContainer} >
                                <TouchableOpacity onPress={() => navigation.navigate('DirectorioDetalle', { idDetalle: item["directorio"] })}>
