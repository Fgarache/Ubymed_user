...
export default LabsConfirmarCompra = ({ navigation, route }) => {
    ...
    useEffect(() => {
        ...
        .then((responseJson) => {
            if(responseJson["error"] === true) {
                ...
                let total = parseFloat(responseJson["data"][0]["costo_servicio"]);
                
                responseJson["data"].forEach(element => {
                    total = total + parseFloat(element.costo);
                });
                //calcular nuevo costo por servicio y nuevo costo total basado en las comisiones del cobro en línea
                let costoServicioAdicional = Math.round(parseFloat(responseJson["data"][0]["pcosto_servicio"]) * total / 100);
                total = total + costoServicioAdicional;
                setCostoServicio(parseFloat(responseJson["data"][0]["costo_servicio"]) + costoServicioAdicional);
                setCostoTotal(total);
            } 
        }).catch((error) => {
    ...
    const confirmarCompra = () => {
        ...
        if (!validarTarjeta(numero.replace(/ /g, ""))) {
        ...
        } else if (!lugar || !direccion || !nombre || !numero || !correo){
            setLoading(false);
            setShowModal(true);
            setModalTitle('¡Error!');
            ...
            fetch(global.UBYMED_WS_BASE + 'api/usuario', {
                ...
                body: JSON.stringify({
                    action: "generarCompraLab",
                    usuario: loginContext.userData.usuario,
                    laboratorio: idDetalle,
                    lugar: lugar,
                    direccion: direccion,
        ...
        <ScrollView style={{marginBottom: espacioIOS,}}>
            ...
            <View style={styles.containerServicios}>
                ...
                <View style={styles.formularioContainer}>
                    {
                        dataMostrar.map((item) => {
                            return (
                                <View style={styles.formularioInputContainerLineSup} key={item.nombre_lc}>
                                    <View style={styles.FormularioRow}>
                                        <Text style={styles.FormTextLabelCompra}>{item.nombre_lc}</Text>
                                        <Text style={styles.FormTextLabelLight}>Q{item.costo}</Text>
                                    </View>
                                </View>
                            );
                        })
                ...
                <View style={styles.formularioContainer}>
                    <View style={styles.formularioInputContainer}>
                        <View style={styles.FormularioRow}>
                            <Text style={styles.FormTextLabelCompra}>Servicio Ubymed</Text>
                            <Text style={styles.FormTextLabelLight}>Q{parseFloat(costoServicio + "").toFixed(2) }</Text>
                        </View>
                    </View>
                </View>
                ...
                <Text style={styles.labelPrincipalBodySpaced}>LABORATORIO CLÍNICO</Text>
                <View style={styles.formularioContainer}>
                    <View style={styles.formularioInputContainerCompra}>
                        <View style={styles.FormularioRow}>
                            <Text style={styles.FormTextLabel}>{(dataMostrar.length > 0) ? dataMostrar[0].nombre + " " + dataMostrar[0].nombre_detalle : '---'}</Text>
                        </View>
                        <View style={styles.FormularioRow}>
                            <Text style={styles.FormTextLabel}>{(dataMostrar.length > 0) ? dataMostrar[0].direccion : '---'}</Text>
                        </View>
                    </View>
                </View>
